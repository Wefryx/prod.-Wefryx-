...
const SUPABASE_URL=window.WEFRYX_SUPABASE_URL;
const SUPABASE_KEY=window.WEFRYX_SUPABASE_ANON_KEY;
const PUBLIC_TIMEOUT=7000;

async function publicFetch(path){
  if(!SUPABASE_URL||!SUPABASE_KEY||SUPABASE_URL.includes('YOUR-PROJECT')) throw new Error('Supabase config missing');
  const controller=new AbortController();
  const timer=setTimeout(()=>controller.abort(),PUBLIC_TIMEOUT);
  try{
...
async function loadPortfolio(){
  ...
  try{
    const query='portfolio?select=id,title,duration,category,video_url,thumbnail_url&published=eq.true&order=sort_order.asc.nullslast,created_at.desc&limit=24';
    const data=await publicFetch(query);
    ...
    const card=(x,vertical)=>{
      ...
      if(x.thumbnail_url){
        ...
        img.className='video-poster'; img.src=x.thumbnail_url; img.alt=x.title||'Видео из портфолио';
      ...
      play.addEventListener('click',(e)=>{e.stopPropagation();openVideoModal(x)});
    ...
    shortList.innerHTML='<div class="portfolio-loading">НЕ УДАЛОСЬ ЗАГРУЗИТЬ ПОРТФОЛИО</div>';
    longList.innerHTML='';
  }
}
...
function ensureVideoModal(){
  ...
  modal.id='video-modal'; modal.className='video-modal';
  modal.innerHTML=`
    ...
    <div class="video-modal-dialog" role="dialog" aria-modal="true" aria-label="Просмотр видео">
      ...
      <div class="video-modal-info"><div class="video-modal-title"></div><div class="video-modal-meta"></div></div>
    </div>`;
  document.body.appendChild(modal);
  modal.querySelectorAll('[data-close-video]').forEach(el=>el.addEventListener('click',closeVideoModal));
}
function openVideoModal(x){
  ensureVideoModal();
  const modal=document.getElementById('video-modal');
  const player=modal.querySelector('.video-modal-player');
  modal.querySelector('.video-modal-title').textContent=x.title||'Видео';
  modal.querySelector('.video-modal-meta').textContent=`${x.category==='short'?'Монтаж Reels / короткое видео':'Монтаж длинного видео'}${x.duration?' · '+x.duration:''}`;
  player.src=x.video_url; modal.classList.add('open'); document.body.classList.add('modal-open');
  player.currentTime=0; player.play().catch(()=>{});
}
function closeVideoModal(){
  const modal=document.getElementById('video-modal'); if(!modal)return;
  const player=modal.querySelector('.video-modal-player'); player.pause(); player.removeAttribute('src'); player.load();
  modal.classList.remove('open'); document.body.classList.remove('modal-open');
}
document.addEventListener('keydown',(e)=>{if(e.key==='Escape')closeVideoModal()});

async function loadReviews(){
 const box=document.querySelector('#reviews-list'); if(!box)return;
 try{
  const data=await publicFetch('reviews?select=id,nickname,service_type,rating,review_text,avatar_url&approved=eq.true&order=created_at.desc&limit=3');
 ...
 }catch(e){console.error('Reviews:',e);box.innerHTML='<div class="reviews-empty">НЕ УДАЛОСЬ ЗАГРУЗИТЬ ОТЗЫВЫ</div>'}
}
function escapeHtml(s){return String(s||'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]))}

// Не блокируем отрисовку страницы Supabase-библиотекой. Данные подгружаются сразу после построения DOM.
setTimeout(()=>{loadPortfolio();loadReviews();},50);
